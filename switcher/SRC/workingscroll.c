...
#define SHADLEN(flags)	((flags&SLEN)>>9) //8
...
#ifdef SERDEBUGger
			sprintf(namebuff,"\tmax,min outxy: %d %d %d %d %d %d \t",Face.MinX,Face.MinY,Face.MaxX,Face.MaxY,Out.MinX,Out.MinY);
			DUMPSTR(namebuff);
#endif
...
//
//				BltBitMap(&FBM1,Out.MinX,Out.MinY,&FBM2,Out.MinX,Out.MinY,
//				(Out.MaxX-Out.MinX+1),(Out.MaxY-Out.MinY+1),0xc0,0x1,NULL);
#endif
 		MP->CurChar++;
		}
