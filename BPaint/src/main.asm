...
*
	DUMPMSG	<toasterwait ForceToastCopper>	
...
;MAR91....recopy "my" buffer back to global
	...
	DUMPMSG	<recopy_globals>	
recopy_globals:
	move.b	(a1)+,(a0)+
	dbf	d0,recopy_globals
	DUMPMSG	<afer recopy_globals>
	move.l	GlobalRGBPtr_(BP),d0
	move.l	d0,a0
...
;did the picture get loaded?...if not...bye! 
	...
	DUMPMSG	<setting max w/h>	
	move.w	#MAXWT,BigPicWt_W_(BP)
	move.w	#MAXHT,BigPicHt_(BP)

	move.l	#'BOOT',d0		;d0=LONG # (equiv 4 ascii) ('boot' code(s))
	bsr	ZipKeyFileAction 	;main.key.i, reads keyfile for startup cond
	DUMPMSG	<after ZipKeyFileAction>		
...
;1$
	...
	cmp.l	#0,MsgPtr_(BP)		;TEST ADDED 110395DEH
	beq	nomsg


 IFEQ	0 	*KEYWORD  brought back in 020295
...
*	
	...
	xjsr	NewMsg_Handler		;gadgetup case	*KEYWORDX
	bra	nomsg	
...
CKIntAct:
	...
	bne	gotmsg
	clr.l	MsgPtr_(BP)
	xjsr	NewMsg_Handler		*KEYWORDX
	bra	nomsg

BeginPalProp:
	move.l	gg_UserData(a1),PalGadAct_(BP)
	xjsr	NewMsg_Handler		;gadgetdown case	*KEYWORDX
	bra	nomsg
...
;;	DUMPMSG	<Calling UpdateCustomGads>
...
379$
	...
	beq.s	oknosho
;;	DUMPMSG	<CALLING ReallyShowPaste>
...
*	xjsr	ShowPalette		;showpal.o, checks/clears FlagNeedShowPal	*KEYWORD
	restart_if_msg			;check for msg AFTER text, palette update
...
;	xjsr	CloseSkinny	;mousertns.o, kills 'rgb' display (if any) MAY1990	*KEYWORD NOT COMOUT IN OLD VER:
...
4$	ext.l	d1
	...
	movem.l	d0/d1/a0,-(sp)
	CALLIB	Intuition,MoveScreen	*KEYWORD
...
*	st	FlagNeedText_(BP)	;next time around, setup frame #
	bra	rawk_rts			;no lup array, numeric digitis, framestore

		;NOTE: need arexx framestore digit stuff...
notin_framestore:
	move.l	KeyArrayPtr_(BP),d1
	beq	rawk_rts			;no lup array
...
scanktab:			;a0 steps thru table, looking for d0 match
	...
	beq	rawk_done ;rts	;outta here...enda table
...
fndkc:	;"found key code", build a fake 'msgptr' setup based on found code
	...
	move.l	(a1),d0		;the 1st 4 ascii
;;	DUMPMEM	<doaction on this!>,(A1),#64	
...
;ensure SCREEN(not bitmap) not bigger than 'normal' (booo....)
	...
	DUMPREG	<BigPicWt/d1,BigPicHt/d2,NormalWidth/d3,NormalHeight/d4>
	tst.b	FlagLaceNEW_(BP)
	beq.s	1$
	add.w	d4,d4
1$	cmp.w	#600,d4
...
dcy	equ	-34		*-35
	...
	move.w	#384,ns_Width(a0)	; test for aa	
	move.w	#480,ns_Height(a0)	; test for aa
	move.w	#dcy,ns_TopEdge(a0)	; test for aa
	move.w	#dcx,ns_LeftEdge(a0)	; test for aa
...
mywinf	equ	BORDERLESS!REPORTMOUSE!NOCAREREFRESH!RMBTRAP
	...
	DUMPMSG	<after Crit>
	xjsr	FreeUnDo		;get ridda the 'old' one
	DUMPMSG	<after FreeUnDo>
	xjsr	FreeUnDoRGB		;if any...LATEMAY1990
	DUMPMSG	<after FreeUnDoRGB>
getnewu:
	sf	FlagBitMapSaved_(BP)	;forces saveundo to work
	DUMPMSG	<before SaveUnDo>
	xjsr	SaveUnDo		;allocs undo bitmap as needed		<<<<<<< Enforcer hits!	
	DUMPMSG	<after saveundo>
...
humptdumpty:
	xref	PasteBitMap_
	DUMPMEM	<pbm>,PasteBitMap_(BP),#64	
	DUMPMEM	<RGBbm>,(a4),#64	
	rts
...
1$
	...
	xdef	biglybigly
biglybigly:
;;		DUMPMEM	<COLORMAP>,(A1),#64
	DUMPREG	<A1,2BM D2W  D3H>
	rts
