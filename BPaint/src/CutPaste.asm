...
Paste_Again:	;come here when again gadget hit, already have brush
	movem.w	mydraw,d0/d1	;x,y
	DUMPREG	<paste again at d0/d1>
...
**SEEMS VERY IMPORTAINT!!  
	ASL.W	MyDrawX_(BP)		; 1X MODE NEED TO 2X STAMP POINT FIX BR030595
	MOVE.W	paste_offsetx_(BP),D0	; MUST SUBTRACK OFFSET THOUGH	BR030595
	SUB.W	D0,MyDrawX_(BP)		; ^^^^^ 
	MOVEQ	#0,D0

	DUMPREG	<MyDrawX-D2>	
	DUMPMEM	<paste_offsetx>,paste_offsetx_(BP),#32
;	DUMPMEM	<paste_leftblank>,paste_leftblank_(BP),#32
...
;;	BSR	BIGBIRD			;DEBUGING IN TIGHT SPACES
1$
...
3$
	...
	xjsr	RePaint_Picture		;scratch.o ;Again
	;;st	FlagNeedShowPaste_(BP)	;AUG311990...shows brush after repaint...
	;;bra	ReallyShowPaste		;...show brush after painting...AUG311990
	bra	ForcedShowPaste		;...show brush after painting...AUG311990
	;rts		;Paste
...
Paste:
	...
	move.l	MyDrawX_(BP),mydraw
;	movem.w	d0/d1,last_paste_x_(BP)
;;	move.l	MyDrawX_(BP),save_display_x_(BP)
...
Paste:	
	...
	move.w	paste_leftblank_(BP),paint_leftside_(BP) ;USED BY REPAINT
	move.w	MyDrawX_(BP),d2		;'center' of mousepointer
	DUMPREG	<in paste mydrawx-d2>
	sub.w	paste_offsetx_(BP),d2	;leftedge to blit whole brush to
	DUMPREG	<In paste -pasteoffset-d2,  >
	bpl.s	1$
...
1$
	...
	subq.w	#1,d6	;...-1=rightside pixel	#
	cmp.w	BigPicWt_W_(BP),d6
	bcs	199$	;not off right side...
	move.w	BigPicWt_W_(BP),d4
	sub.w	d2,d4	;-toX = new width
	BSR	BIGDEAL
;;	DUMPREG	<OFF RIGHT SIDE?>
199$
;;	DUMPREG	<OFF RIGHT SIDE?>
	clr.w	paste_clipy_(BP)	;this time only (used by repaint)
	move.w	MyDrawY_(BP),d3		;"to" y
...
MostlyEndCutPaste:	;subr...MACRO
	...
	st	FlagNeedGadRef_(BP)	;signal to redraw gadgets, reset brush JUNE291990
no_old_brush:
	...
	;JUNE291990;st	FlagNeedGadRef_(BP)	;signal to redraw gadgets, reset brush
...
;	tst.b	FlagToast_(BP)			;KEYWORD   <THIS IS DISABLED NOW TO ALLOW TESTING OF THE CUTTING.... 
;	beq.s	1$
;	st	FlagDisplayBeep_(BP)
;	rts
1$
	bsr.s	MostlyEndCutPaste	;doesn't kill cpundo:xtra cutpaste undo
		...
		;AUG161990
;AUG161990;		;FORCE the 'Scis' action code to *destroy* (copy to) the undo buffers
;AUG161990;		;...this should fix rgb<->ham bitmap problems,
;AUG161990;		;...problems with "UnDo" "Scis" "Redo"
;AUG161990;		;...problems with "Scis" "UnDo" "Redo"
;AUG161990;	;...but not if ROTATE effects...
;AUG161990;	xref EffectNumber_
;AUG161990;	cmp.b	#4,EffectNumber_(BP)	;rotate+90
;AUG161990;	beq.s	119$
;AUG161990;	cmp.b	#5,EffectNumber_(BP)	;rotate-90
;AUG161990;	beq.s	119$
;AUG161990;	xjsr	SetAltPointerWait	;non-interruptable....
;AUG161990;	xjsr	SaveUnDo		;memories.asm
;AUG161990;	xjsr	SaveUnDoRGB		;rgbrtns.asm
;AUG161990;119$
...
noundo_ok:
	...
	xjsr	AirBrushoff
;	sf	FlagAir_(BP)
	sf	FlagFillMode_(BP)	;'new' fill off (user can switch on)

	;AUG161990;xjmp	SetPointerCut		;may05'89....helps with kybd 'B'rush, 'A'gain
	xjsr	SetPointerCut		;may05'89....helps with kybd 'B'rush, 'A'gain
...
;AUG161990;	;...but not if ROTATE effects...
	xref EffectNumber_
	cmp.b	#4,EffectNumber_(BP)	;rotate+90
	beq.s	119$
	cmp.b	#5,EffectNumber_(BP)	;rotate-90
	beq.s	119$
	xref FlagOpen_
	tst.b	FlagOpen_(BP)	;loading (a brush)? AUG281990
	bne.s 119$ ;AUG281990
	;no need...;xjsr	SetAltPointerWait	;non-interruptable....
	xjsr	SaveUnDo		;memories.asm
...
;;_SwapSuperScreen:
;;	xjmp	SwapSuperScreen	;memories.asm
;;	;rts

_UnDoRGB:
	xjmp	UnDoRGB		;rgbrtns.asm
	;rts

abort_cut_and_clear:	;AUG221990
	xjsr	CopySuperScreen	;undo the "ham lines"
...
CutorPaste:	;copy bitmaps (using 7th plane of paste map for brush mask)
	...
	DUMPMSG	<Begin CutorPaste>
	;AUG161990;tst.l	PasteBitMap_Planes_(BP)	;do we already have a brush?
	;AUG161990;bne	Paste			;already allocated

		;HAVE a paste-down position? AUG161990
	tst.l	PasteBitMap_Planes_(BP)	;do we already have a brush?
	beq.s	10$
	tst.w	last_paste_x_(BP)
	bpl	Paste			;"have" a good "PASTE" location?
	st	FlagDisplayBeep_(BP)
	rts
10$
...
;JULY06;	bsr	MostlyEndCutPaste
	...
	cmp.b	#7,EffectNumber_(BP)	;7=range mode	;JULY181990
	beq.s	8$
...
8$	xjsr	MarkedUnDo		;removes 'cutout' edge from screen
	...
	xjsr	CheckCancel
	bne	abort_cut_and_clear
	DUMPMSG	<not abort_cut_and_clear>
...
;help out text brushes...not quite wide enough...AUG271990
	...
	add.w	#2,d2		;ending 'x'
	add.w	#2,d4		;width
	DUMPMSG	<adding 2 to X and W>
notextadj:

	;AUG161990
	;IF user did not select "brush load", then force an extra "undorgb"
	pea	_UnDoRGB(pc)	;AUG161990.....helps match up ham/rgb images

	xref FlagBrush_		;file requester setup for brush load?

  IFC 't','f' ;MAR91
	tst.b	FlagBrush_(BP)		;file requester setup for brush load?
	beq.s	001$			;nope....continue "normal"
	pea	_UnDoRGB(pc)		;AUG161990.....helps match up ham/rgb images
001$
  ENDC ;MAR91
...
;	xref FlagBrush_			;file requester setup for brush load?
	tst.b	FlagBrush_(BP)		;file requester setup for brush load?
	bne.s	002$			;nope....continue "normal"
	pea	_SwapSuperCPUnDo	;memories.o
002$
	;;;pea	_UnDoRGB(pc)		;AUG161990.....helps match up ham/rgb images
...
bbbe1	move.w	d0,paste_x_(BP)
	move.w	d4,paste_leftblank_(BP)

	move.w	paste_width_(BP),d4
	add.w	#1,d4			;;testing 070395 DEH 
	lsr.w	#1,d4
	...
	DUMPMSG	<after AllocPaste>		

	tst.l	PasteBitMap_Planes_(BP)	;did get requested memory?
	beq	abort_cut	
	;(label at top of this module)

	DUMPMSG	<Bamb!!!>   *** doesnt get here!KEYWORD** does now!!030395

	pea	_UnDoRGB(pc)		;do this after everything else, kludgey, AUG071990

;	tst.b	FlagText_(BP)		;june271990...cutting a text brush?
;	bne.s	yep_cuttext
;	xjsr	UnDoRGB			;june271990....fixes rgb-undo bug when cutting...
;yep_cuttext:


	DUMPREG	<InitBitPlanes>	
...
28$
	...
	xref FlagOpen_
	tst.b	FlagOpen_(BP)
	beq.s	288$			;not loading a file
	tst.b	FlagBrush_(BP)		;loading, but a "brush" load?
	beq.s	29$			;not a brush load, continue...
288$
	tst.b	FlagText_(BP)		;AUG271990...dont saveundorgb if text brush
	bne.s	29$
	xjsr	SaveUnDoRGB		;AUG121990...simulate, in rgb, the hambitmap stuff
29$
...
30$
	...
	xref	LastRepaintHt_		;AUG281990
	clr.w	LastRepaintHt_(BP)	;AUG281990
...
_CopySuperScreen:
	...
	;xjsr	SaveUnDoRGB		;june271990...simulate, in rgb, the hambitmap stuff
	;xjsr	UnDoComp		;composite.asm...helps w/composite plot
	;movem.l	(sp)+,d0-d7/a0-a6	;YUCK ....CLEANUP
	;june271990...no need at all?

	;AUG151990....
	;ensure that rgb data matches "what we do to the ham images..."
	movem.l	d0-d7/a0-a6,-(sp)	;YUCK ....CLEANUP
	xjsr	UnDoRGB			;rgbrtns.asm
	xjsr	UnDoComp		;composite.asm...helps w/composite plot
	movem.l	(sp)+,d0-d7/a0-a6	;YUCK ....CLEANUP
...
ReallyShowPaste: ;guarantees that ScreenBitMap shows current pastebrush
	...
	;bne	UnShowAndFreeDouble	;nope...outta here...
...
1$
	...
	;30JAN92;bne	UnShowAndFreeDouble	;UnShowPaste	;yep...outta here...
	bne	UnShowPaste		;30JAN92...only "unshow", don't FreeDouble
	...
	;AUG291990;beq.s	10$
	BNE	UnShowPaste		;remove cutout brush when/for picking....
	xref LastM_Window_
	xref GWindowPtr_
...
ForcedShowPaste:			;AUG311990....for after repaint
	xjsr	SetHigherPriority	;(background ok), main loop slows it
	...
	beq.s	use_mouse
	xjsr	grab_xyrxarg		;gadgetrtns.o, sups MyDrawX,Y
...
gots_xy:
	;xjsr	GridCoordinate		;mousertns, DigiPaint PI
	;;not needed?;
	;movem.w	d0/d1,MyDrawX_(BP)	;kludge?
	move.w	D0,save_display_x_(BP)	;parm for 'ReSeePaste' in blits.o
	move.w	d1,save_display_y_(BP)
	...
	cmp.w	sc_BitMap+bm_BytesPerRow(a0),d2	;past rt edge?
...
;doesn't help;		;29JAN92....help w/double buffering...?
;doesn't help;	xjsr	CleanupMemNoWb	;cleans up, but doesnt force flag closewb memories.asm
;doesn't help;	bsr.s	DoDouble
;doesn't help;	bne.s	enda_showpaste	;else no memory for double scrn, do old way
...
;DoDouble:	;allocate CHIP backup of screen, for building 'final blit'
 DLBL DoDouble
	movem.w	save_display_x_(BP),d0/d1	;DigiPaint PI...
	xjsr	GridCoordinate
	movem.w	d0/d1,save_display_x_(BP)
