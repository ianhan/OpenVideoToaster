...
setscreentoo:
	...
	asl.w #6,d0   ; d0=d0*32 (plot routines are now 64 bytes eatch)
	xref Plot_Routine_Start8
	lea	Plot_Routine_Start8,a6
	lea	0(a6,d0.w),a6
...
100$:		;JUNE05
	...
	bcc.s	001$
	moveq	#15,d0
001$:	asl.w #6,d0   ; d0=d0*32 (plot routines are 26 bytes + 6 cnop's each)
	lea	Plot_Routine_Start8,a6
	lea	0(a6,d0.w),a6
...
setms_loop:
	...
	movem.l	(6*4)(a0),d3-d4	; now are 8 planes
	move.l	(5*4)(a0),a5	; last bitplane adr (now sixth)
	move.l	(a0),a0		; first bitplane adr
	jsr	(a6)		;plot_routine_start
	MOVEM.l	(sp)+,d1/d2-d4/a0/a5
09$:
	subq	#1,d3	;bitnumber countdown
