* DoBrush.asm 	;INCLUDES ps:PaintCode.i.asm ps:DoBrushLine.1.asm
* savearray note: s_stfx_bottom is used (in place of s_PlaneAdr)
*                 for a .word size offset within a line (ham bitmap)
...
; xjsr DebugMe2
	...
	move.w	d0,s_stfx_bottom(a6) ;s_PlaneAdr(a6)	;1st entry	;OFFSET WITHIN ROW
	moveq	#0,d0
...
IsColor:	;find which color
	...
	asr.b	#6,d1	;strip off shade#, leave 2 bit color#
	subq.b	#2,d1
	bcs	Isblue	;color # 1
...
redIsColor:	;find which color
	move.b	d0,d1	;get pixel
	asr.b	#6,d1	;strip of shade#, leave 2 bit color#
	subq.b	#2,d1
...
redIsgreen:
	andi.b	#$3F,d0	;strip of HAM color select, leave 6 bit shade
	move.b	d0,s_green(a6)	;save the actually seen value
...
loop_get_savearray:
 ...
 ; get_superb_pixel
  ...
  get_superb_quick	;may02'89
	...
	move.w	d1,s_stfx_bottom(a6) ;s_PlaneAdr(a6)	;* BUILD SaveArray_(BP) *
...
flagbrushbitloop:
	moveq	#0,d1
	move.w	s_stfx_bottom(a6),d1 ;s_PlaneAdr(a6),d1	;offset within line
...
18$
	;move.b	DisplayedRed_(BP),s_Paintred(a6)
	...
	move.W	Paint8red_(BP),d4
	move.B	d4,s_Paintred(a6)
	move.W	Paint8green_(BP),d4	;this is a WORD sized var (byte valid)
	move.B	d4,s_Paintgreen(a6)
	move.W	Paint8blue_(BP),d4
	move.B	d4,s_Paintblue(a6)
...
19$
	...
	subq	#2,d0	;WHY?.....JULY191990....moves rubthru "left"...;#3,d0
...
1$:	lea	2(sp),sp
	...
	;june301990;move.w	Predold_(BP),s_Paintred(a6)
	;june301990;move.b	Pblueold_(BP),s_Paintblue(a6)
		;june301990
		;getold returns d0.long=8 bit colors
	move.B	s_Paintred+3(a6),d0	;dont clobber 4th byte...
...
end_of_rub_thru:
  LIST
	...
	include "ps:paintcode.i"	;PAINT CALCULATIONS

  NOLIST
		;CALL DETERMINE + SAVE 'BESTPEN' FOR QUICKDRAW
	move.l	(a6),Pred_(BP)	;THIS WORKS....june2490....but why?
	;move.l	(a6),d0		;8 bits calc'd by paintcode, >>4 for 4 bits
	;asr.l	#4,d0		;...."determine" only uses ham- 4 bits
	;and.l	#$0f0f0f00,d0
	;move.l	d0,Pred_(BP)
...
; xjsr DebugMe5
	...
	movem.l	d3-d4/BP,-(sp)		;STACK
	MOVE.L	line_offset_(BP),-(sp)	;STACK offset to start of "this" line
	movem.l	ScreenBitMap_Planes_+(6*4)(BP),d3/d4	;prepare for FastPlot
...
continue_plotting:	;-----
	...
	move.w	s_stfx_bottom-s_LastPlot(a6),d1	;byte # FROM START OF PLANE (s_PlaneAdr)
	;ADD.L	line_offset_(BP),d1
	ADD.L	(sp),d1
	move.b	s_BitNumber-s_LastPlot(a6),d2	;bit # in byte (S_BitNumber)
	xjsr	FastPlot_GotAdr8		;22 jsr, 16 RTS, ~200 for routine
