...
;	xjsr	SetEntireScreenMask	;memories.o, whole screen mask
...
;LOOP DE-PLOT a ham line into SaveArray given:
	...
	tst.b	Global_Flood_(BP)	;global flood on?
	bne	.globalflood000		yes, skip nonglobal offsets
	sub.w	MyDrawY_(BP),d0		*ng
.globalflood000
*
	move.w	d0,-(sp)		;loop counter MyDrawY'th row to bottom...
*
	tst.b	Global_Flood_(BP)	;global flood on?
	bne	.globalflood001		yes, skip nonglobal offsets
	move.l	bytes_per_row_(BP),d0	*ng
	mulu	MyDrawY_(BP),d0		*ng
	move.l	d0,linecol_offset_(BP)	*ng
.globalflood001	

for_eachrow:
	move.w	#480,d1			;get new linecol offset for 752 size image
	sub.w	(sp),d1			;why is mask 768 wide?   
*	tst.b	Global_Flood_(BP)	;global flood on?
*	beq	.globalflood003
*	add.w	MyDrawY(BP),d1	
*.globalflood003
	mulu	#752,d1
	bsr	GetLineOfRGB		;fills savearray with rgb values
...
;note: (d1 && $ff00) not= 0 if any pixels set//marked
	tst.b	Global_Flood_(BP)	;Global on?
	bne	.notcancel001		;yes, dont cancel at all.
...
*	move.l	linecol_offset_(BP),d1
	move.w	(sp),d1			;get new linecol offset for 752 size image
	sub.w	#1,d1
	mulu	#752,d1
	bsr	GetLineOfRGB		;fills savearray with rgb values
...
*	bsr	Copy_2_Mask
 ...
 ifd SaveFloodMask
	bsr	SaveMask		;TEST! ONLY	
 endc
	tst.b	Global_Flood_(BP)	;global flood on?
	bne	.noflood_GON	 	;skip if global is on		
	CALLIB	Graphics,Flood		;COMENT OUT FOR FULLSCREEN AREA FILL.
.noflood_GON	
...
;'AND'/'OR' together the 2 brush bitplanes...create final mask
	xref PlaneSize_
...
*	
	...
	bne	Pro_maskG		;just go do golbal	
	bra 	Pro_maskNG
after_fm
	rts
...
*	add.w	d1,d1			;4
 ...
 ifeq	1
	move.b	s_Paintred(a6),d1
	lsr.b	#4,d1
	move.b	d1,s_red(a6)
	move.b	s_Paintgreen(a6),d1
	lsr.b	#4,d1
	move.b	d1,s_green(a6)
	move.b	s_Paintblue(a6),d1
	lsr.b	#4,d1
	move.b	d1,s_blue(a6)
 endc
...
save_ham_colors:
	...
	lea	s_SIZEOF(a6),a6 	
;next record in SaveArray, make it current
	dbf	d3,loop_get_savearray
...
crepeat2:			;  REPEAT
	...
	bsr	TestPixel	;current+SrcDir pixel set? (verify Sdir<>neg)
	beq.s	else1		;no,
...
;****** NewFlood/GetLineOfRGB *****************************************
;
;   NAME
;	GetLineOfRGB -- Reads bytes/pixels from bytemap. 
;
;   SYNOPSIS
;	GetLineOfRGB(LineOffset)  <--- REALLY A BYTE OFFSET INTO A BYTEMAP
;		     d1.w	
;
;   FUNCTION
;	Goes to offset into undo buffer and gets a row of bytes/752 and
;	copy shifts them down to 4 bit values.
;	The resulting values are placed in the SAStartRecord_ Array.
;
;   INPUTS
;	LineOffset	- an offset from the bottom of the bytemap 
;			  in a 480 line bytemap 480 would be the top line.
;	
;   RESULT
;	are in save_array s_SIZEOF. 
;	$00000000 <- first long word in each entry.
;	 rrggbbxx	
;
;   EXAMPLE
;
;   NOTES
;	Uses the UnDoRGB ByteMap.
;	Should be passing ByteMap addresses.
;	Width of from plane set at 752.
;	Height fixed at BigPicHt.
;
;   BUGS
;	
;   SEE ALSO
;	SAVERGB.i for save_array struct.
;
;****************************************************************************
	xdef	GetLineOfRGB
GetLineOfRGB:
...
*	
	lea	UnDoRGB_(BP),a0
*
	move.l	bm_Planes(a0),a1		;red plane address
	add.l	d1,a1
	move.l	bm_Planes+4(a0),a2		;green plane address
	add.l	d1,a2
	move.l	bm_Planes+8(a0),a3		;blue plane address
	add.l	d1,a3
	

*	move.w	ppix_row_less1_(BP),d0		;Paint PIXels per row, minus one
*	move.w	(a0),d0				;WIDTH IN BITMAP_STRUCT
	move.w	#751,d0
1$	
	move.b	(a1)+,d1			;get red
	lsr.b	#4,d1				;convert to 4 bit
	move.b	d1,(a6)				;store in array
*
	move.b	(a2)+,d1			;get green
	lsr.b	#4,d1				;convert to 4 bit
	move.b	d1,1(a6)			;store in array
*
	move.b	(a3)+,d1			;get blue
	lsr.b	#4,d1				;convert to 4 bit 				
	move.b	d1,2(a6)			;store in array
*	
	LEA	s_SIZEOF(A6),A6			;INC ARRAY PTR
	dbf	d0,1$	
*
...
*
 ifeq	1
	DUMPMEM	<RASTPORT>,(A1),#rp_SIZEOF
*	
	MOVE.L	rp_TmpRas(A1),A0
	DUMPMEM	<TEMPRASTER>,(A0),#rp_SIZEOF
*	
	MOVE.L	rp_BitMap(A1),A0
	DUMPMEM <BITMAP>,(A0),#bm_SIZEOF+32
*
	MOVE.L	bm_Planes(A0),A1
	DUMPMEM <BITPLANE1>,(A1),#1500
*
*	MOVE.L	bm_Planes+4(A0),A1
*	DUMPMEM <BITPLANE2>,(A1),#500
*
 endc
*
...
*	lea	BigPicRGB_(BP),a1
	lea	UnDoRGB_(BP),a1
	DUMPMEM <BITMAP>,(A1),#bm_SIZEOF+32
	movem.l	(sp)+,d0-d7/a0-a4
	RTS	
 endc
...
GOGLALAYA:
	movem.l	d0-d7/a0-a4,-(sp)
	move.l	SAStartRecord_(BP),a6		
	DUMPMEM	<SAS_RECORDS>,(A6),#s_SIZEOF*16
	movem.l	(sp)+,d0-d7/a0-a4
	RTS	
 endc
...
DumpSAS:
	movem.l	d0-d7/a0-a4,-(sp)
	DUMPMEM	<SAS_RECORDS>,16*100(A6),#16
	movem.l	(sp)+,d0-d7/a0-a4
	RTS	
 endc	

 ifeq	1	* from #David_Data to A0 * 23040 words.
Copy_2_Mask:
	movem.l	d0-d7/a0-a4,-(sp)
...
SaveMask:
	...
	lea	TextMask_RP_(BP),A1	;SYNONYM FOR 'RASTPORT of DRAWING MASK'
	move.l	rp_BitMap(A1),A1	;bitmap

	MOVE.L	D6,A6
	lea	maskfile,A0		filename
	lea	cmap,a2			colortable
	moveq	#0,d0			flags ~compressed
*
